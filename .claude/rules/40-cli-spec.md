...
- **カテゴリ別カラー + 記号化 実装済み 🟢**(`tools/cb_keymap.py` + `cb_keymap_tui.py`, `90` 続33,
  issue #37 完了): keymap グリッドを**キー種別で着色**(mod=cyan / F+メディア=pink / nav=green /
  英数=white / 記号=grey / 0x92 ベンダー=purple、未割当=dim)。modifier は **⌘⌥⌃⇧ + L/R**、矢印は
  **←→↑↓** にコンパクト記号化(`decode()` と skeleton 既定の両方)。着色は **`cell_geometry()` 駆動の
  後段**で実装 — `render()` はプレーンのまま返し、`show` は ANSI を、TUI は Rich style を**セル span に

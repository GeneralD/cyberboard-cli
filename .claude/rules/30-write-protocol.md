...
### 5a. トランザクション意味論 = 全置換(部分書込不可)🟢(実機確定 2026-06-22, `90` 続8)

- **`JSON_START` が設定フラッシュ領域を消去**する。各セクションが自領域だけ再書込し、
  **送らなかったセクションは消去状態(`#FF…`)で残る**。= **1 トランザクション = 設定全体の置換**。
  実証: LED セクションだけ送り `key_layer` を省略 → keymap が全 `#FFFFFFFF` に消えた
  (`experiments/partial-write/`)。**部分書込は非対応** → 分離管理は **read→merge→フル書込**で実現(`40`)。
- ⚠ **書込直後の read-back は settle 遅延が必要**: `JSON_END` 直後に `[6,9]` で読むと全 `#00000000`
  (commit 未完了)。**~2s 待つ**と正値(1400/1400)。検証読戻し前に待つこと。
